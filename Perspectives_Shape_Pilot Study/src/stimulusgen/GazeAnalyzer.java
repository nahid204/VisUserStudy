...
public class GazeAnalyzer extends Viewer2D {
	...
	public GazeAnalyzer(String name) {
		...
		try {
			Property<PFileInput> p = new Property<PFileInput>("AddUserData", new PFileInput())
					{
						...
						protected boolean updating(PFileInput newvalue) {
							loadUserData(((PFileInput)newvalue).path);
							return true;
						}
					};
					...
					try{
						File file = new File(((PFileOutput)newvalue).path);
						 PrintWriter bw = new PrintWriter(new FileWriter(file));
							...
							int sindex = aggregateUser.stimuli.indexOf(stimuli.get(selected));
							for (int i=0; i<aggregateUser.fixations.get(sindex).size(); i++)
							{
								Fixation f = aggregateUser.fixations.get(sindex).get(i);
								String prov = aggregateUser.fixationProvenance.get(sindex).get(i);  
			...
			Property<PFileOutput> agLayer = new Property<PFileOutput>("AgrLayer", new PFileOutput())
					{
				...
				if (layer != null)
				{
					...
					try {
						...
						System.out.println("imagewritten " + ImageIO.write(layer, "PNG", file));
					} catch (IOException e) {
	...
	{		
		...
		/*GazeHeatmap gh = new GazeHeatmap(stimulusImage.getWidth(), stimulusImage.getHeight(), fixations);
		System.out.println("create vis 3 ");
		gh.computeHeatmap(70);
		layer = gh.drawHeatmap(0, 1.);
		g.drawImage(layer,0,0,null);
		System.out.println("create vis 4 ");*/
			...
			File f = new File(filename);
			 ...
			 while ((s = br.readLine()) != null)
			 {
				...
				if (s.startsWith("UserEvent"))
				{
					...
					if (currentStimulus.imagePath.indexOf("curve") > 0)
				...
				else if (s.startsWith("Fixation") && currentStimulus != null)
				{
					...
					if (fix.duration > 0)
						...
						System.out.println("removing negative");
				}				
